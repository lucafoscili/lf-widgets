...
export class LfChart implements LfChartInterface {
  ...
  #createSeriesData() {
    const { stringify } = this.#core.data.cell;
    const { find } = this.#core.data.column;
  ...
  #updateThemeColors() {
    ...
    } = this.#core.theme.get.current().variables;
    ...
    this.#adapter = createAdapter(
      {
        ...
        columnById: (id: string) =>
          this.#core.data.column.find(this.lfDataset, { id })[0],
        manager: this.#core,
        mappedType: (type) => {
          switch (type) {
            case "area":
        ...
        seriesColumn: (series) =>
          this.#core.data.column.find(this.lfDataset, { title: series }),
