...
export class LfCore implements LfCoreInterface {
  #LISTENERS_SETUP = false;
  assets: { get: LfCoreComputedGetAssetPath; set: LfCoreSetAssetPath };
   ...
   */
  addClickCallback = (cb: LfCoreClickCb, async?: boolean) => {
   ...
   */
  removeClickCallback = (cb: LfCoreClickCb) => {
  ...
  sanitizeProps<P extends { [key: string]: any }, C extends LfComponentName>(
    ...
    const ALLOWED_ATTRS = new Set<string>(LF_CORE_ALLOWED_ATTRS);
    const ALLOWED_PREFIXES = new Set<string>(LF_CORE_ALLOWED_PREFIXES);
